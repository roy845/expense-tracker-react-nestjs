...
const useAssignRoles = () => {
  ...
  const { assignRolesToUser } = useRolesService();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await assignRolesToUser(selectedUser._id, uppercasedRoles);
      toast.success(`Roles ${selectedRoles.join(", ")} assigned successfully!`);
    } catch (error: any) {
      const message =
        error?.response?.data?.errors?.[0]?.message ||
        error?.message ||
        "Failed to assign roles.";
      toast.error("Error assigning roles: " + message);
    }
  };
  return {
    fetchUsers,
